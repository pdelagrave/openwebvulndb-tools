...
class SerializeTest(TestCase):
    ...
    def test_read_and_write_minimal(self):
        ...
        schema = MetaSchema()
        as_string = serialize(schema, plugin)
        ...
        found_back = schema.loads(as_string)
    ...
    def test_popular_mark(self):
        ...
        data = serialize(MetaSchema(), plugin)
    ...
    def test_serialize_reference(self):
        ...
        self.assertEqual('{"type": "osvdb", "id": "12345"}',
                         serialize(ReferenceSchema(), ref, indent=None))
        ...
        with self.assertRaises(ValidationError) as context:
            ReferenceSchema().loads(serialize(ReferenceSchema(), ref))

        self.assertTrue('Either id or url is required.' in str(context.exception))
    ...
    def test_affected_version(self):
        ...
        self.assertEqual('{"introduced_in": "1.2.3", "fixed_in": "1.3.4"}',
                         serialize(schema, ver, indent=None))
        ...
        ver = VersionRange()
        with self.assertRaises(ValidationError) as context:
            schema.loads(serialize(schema, ver))

        self.assertTrue('Either introduced_in or fixed_in is required.' in str(context.exception))
    ...
    def test_serialize_vulnerability_minimal(self):
        ...
        self.assertEqual('{"id": "1234", "title": "Multiple XSS"}', serialize(schema, vuln, indent=None))
    ...
    def test_serialize_vulnerability_cvss(self):
        ...
        self.assertEqual(expect, serialize(schema, vuln, indent=None))
        data = schema.loads(expect)
    ...
    def test_serialize_all_values(self):
        ...
        data = serialize(schema, vuln, indent=None)
    ...
    def test_serialize_vunlerability_list(self):
        ...
        data = serialize(schema, vuln_list)
